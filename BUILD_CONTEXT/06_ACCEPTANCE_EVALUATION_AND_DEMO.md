...
**`AC-P0-08-A — Approve`**  
Given `READY_FOR_REVIEW`, when the manager selects `Approve action`, reads the no-external-action confirmation, and confirms, then one manager-decision event and one simulated-application event are appended atomically and state becomes `APPROVED`.
...
| `2:50–3:20` | Click **Approve action**; pause on confirmation; click the confirmation action once. | Confirmation says no order/outreach/notification occurs; then `Action completed` and `No external action was taken.` | “The agent cannot execute this. A manager explicitly approves, and even then the action changes only this synthetic run.” |
